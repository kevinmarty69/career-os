...
export async function POST(
  ...
  try {
    ...
    const result = await approveRecruiterStrategy(
      session,
      runId,
      await readBoundedJson(request, MAX_APPROVAL_BYTES),
    ...
    if (error instanceof PayloadTooLargeError)
      return new Response('Strategy approval payload too large.', {
        status: 413,
      });
    if (error instanceof RunConflictError)
      return new Response('Strategy approval already differs.', {
        status: 409,
      });
    if (error instanceof ZodError || error instanceof RunRejectedError)
      return new Response('Strategy approval rejected.', { status: 400 });
    return new Response('Strategy approval unavailable.', { status: 503 });
  }
}
