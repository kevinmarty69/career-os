...
- Notifications also expose unresolved numeric source conflicts, due tasks/follow-ups, active current links expiring within 48 hours, and completed runs. Reminders query the whole workspace (first 50 overdue, with overflow disclosed); completed tasks and deleted applications are excluded. Run/link activity retains the existing recent-data window. Failed reminder reads do not hide other events. Read receipts do not complete tasks or identify anonymous link visitors.
...
- LinkedIn archive import now reads only `Positions.csv` locally in the existing Web Worker. Positions, dates and descriptions retain their record locator and remain declared, private, and subject to explicit human review. Messages, contacts and other archive contents are not decompressed. ZIP preflight reuses the DOCX archive limits; encrypted/unsafe/ambiguous packages fail closed. Files over 4 MiB must be reduced to `Positions.csv` on the user's device. Supported CSV columns are `Company Name`, `Title`, `Description`, `Started On`, `Finished On`; maximum 40 positions and 1,000 characters per position, with an explicit error rather than silent truncation. No account synchronization or LinkedIn network request is involved.
...
- The isolated CI database gate now also runs the handoff memory/interview/history smoke and PostgreSQL/TypeScript correction parity check on every push. The synthetic-write opt-in is set only by the disposable native CI stack; local Supabase smoke execution still requires explicit opt-in.
- LinkedIn archive: formatting/lint/types and unit suite pass; production build and 14 import desktop/mobile checks pass. The browser test exercises a real local ZIP worker, excludes a synthetic messages file and saves only after human approval to an intercepted API. This is not a live LinkedIn synchronization or a new Supabase persistence certification.
- LinkedIn follow-up: GitHub found the new instructions inheriting a three-column form layout on tablets (196 of 198 checks passed). Both integration instruction cards now reuse the existing stacked layout; a fresh production build and all 44 import/screen-contract desktop/mobile checks pass, including that tablet regression. English screenshots are generated from the synthetic ZIP worker test.
...
- Notification follow-up: 190 desktop/mobile checks pass, including reminder read-failure recovery and links to the actual timeline/source/link screens. Real Supabase smoke validates due/future/completed task filtering and cross-user isolation. GitHub caught the missing adaptive-interview export category; export contract v14 now includes its durable ledger, and the unchanged schema-coverage test passes against Supabase.
