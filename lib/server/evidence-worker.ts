...
export async function processEvidenceArchivistStep(databaseUrl: string) {
  ...
  try {
    ...
    claimed = await sql.begin(async (tx) => {
      ...
      const [step] = await tx<ClaimedStep[]>`
        select * from app.claim_evidence_archivist_step(${STEP_LEASE_SECONDS})`;
      return step;
    });
    if (!claimed) return { status: 'idle' as const };

    const output = buildEvidenceArchive(
      evidenceArchiveInputSchema.parse(claimed.input),
    );
    outputBuilt = true;
    const artifactId = await sql.begin(async (tx) => {
      await authorizeWorker(tx);
      const [stored] = await tx<{ id: string }[]>`
        select app.complete_evidence_archivist_step(
    ...
    await sql.begin(async (tx) => {
      await authorizeWorker(tx);
      await tx`select app.fail_evidence_archivist_step(
...
async function verifyRestrictedWorkerCredential(sql: postgres.Sql) {
  ...
  join pg_roles target on target.rolname = 'career_evidence_archivist'
...
async function authorizeWorker(tx: postgres.TransactionSql) {
  await tx.unsafe('set local role career_evidence_archivist');
}
